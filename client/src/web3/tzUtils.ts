...
import {
  TempleWallet as NEInvokedWallet,
  TempleDAppNetwork as NEInvokedNetwork,
} from '@temple-wallet/dapp'
...
function useDApp(props: { appName: string }) {
  ...
  const connect = useCallback(
    async (
      network: NEInvokedNetwork = { name: TXNETNAME, rpc: TZNET_RPC },
      opts?: { forcePermission: boolean },
    ) => {
      try {
        /*
                if (!wallet) {
        ...
        const tzLoanDetails = async (contractId: string) => {
          ...
          const loanContract = await tzRet.tezos.wallet.at(
        ...
        return {
          ...tzRet,
          tzLoanDetails,
          reconnect: (n: NEInvokedNetwork) => wallet.reconnect(n),
        }
      } catch (err: any) {
        console.error(`Failed to connect NEInvokedWallet: ${err?.message}`)
        throw new Error(
          err?.message || err?.toString() || 'failed to connect to wallet',
        )
        //return ({error:err});
      }
    },
