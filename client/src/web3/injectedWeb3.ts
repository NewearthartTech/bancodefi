...
export class InjectedWeb3  {
  ...
  connect = async (chainConfig: ChainConfigs, chainId?: string) => {
    ...
    return {web3 :new ethers.providers.Web3Provider(this.injected), account: accounts[0]};
  }

  private readonly ensureCorrectChain = async (chainConfig: ChainConfigs, chainId: string) => {
