...
class Expr:
    ...
    def get(self, item, default_value = None, message = None):
        if default_value is not None:
            return Expr("get_item_default", [self, spExpr(item), spExpr(default_value)], get_file_line_no())
        if message is not None:
            return Expr("get_item_message", [self, spExpr(item), spExpr(message)], get_file_line_no())
    ...
    def open_variant(self, name, message = None):
        if message is None:
            try:
                ...
                result = Expr("open_variant", [self, name, "None" if message is None else spExpr(message)], get_file_line_no())
                self.opens[name] = result
                return result
        else:
            return Expr("open_variant", [self, name, "None" if message is None else spExpr(message)], get_file_line_no())
    ...
    def export(self):
        def ppe(e):
            ...
            if isinstance(e, str):
                return '"%s"' % e
            return str(e)
        line_no = "" if self.line_no is None else "%s " % self.line_no.export()
        if self._f == "invalid":
...
unit = Expr("literal", [Expr("unit", [])], LineNo("", -1))
...
def set_record_layout(expression, layout):
    raise Exception("sp.set_record_layout is deprecated. Please use a command like sp.set_type(.., sp.TRecord(...).layout(..))")

def set_variant_layout(expression, layout):
    raise Exception("sp.set_variant_layout is deprecated. Please use a command like sp.set_type(.., sp.TVariant(...).layout(..))")

def set_type_record_layout(expression, layout):
    raise Exception("sp.set_type_record_layout is deprecated. Please use a command like sp.set_type(.., sp.TRecord(...).layout(..))")

def set_type_variant_layout(expression, layout):
    raise Exception("sp.set_type_variant_layout is deprecated. Please use a command like sp.set_type(.., sp.TVariant(...).layout(..))")
...
class TLambda(TType):
    def __init__(self, t1, t2, with_storage = None, with_operations = False, tstorage = None):
        ...
        if self.with_storage not in [None, "read-write", "read-only"]:
            raise Exception("with_storage parameter can only be None, \"read-only\" or \"read-write\".")

    def export(self):
        tstorage = "None" if self.tstorage is None else self.tstorage.export()
        return "(lambda %s %s %s %s %s)" % (self.with_storage, self.with_operations, tstorage, self.t1.export(), self.t2.export())
...
class Data:
    def __getattr__(self, attr):
        if "__" in attr:
            raise AttributeError("")
        return Expr("attr", [Expr("data", [], get_file_line_no()), attr], get_file_line_no())
...
class SelfProxy:
    def __getattr__(self, attr):
        if attr == "data":
            return Expr("data", [], get_file_line_no())
        else:
            return object.__getattr__(self, attr)

    def __setattr__(self, attr, value):
        if attr == "data":
            sp.set(Expr("data", [], get_file_line_no()), spExpr(value))
        else:
            object.__setattr__(self, attr, value)
...
class Sp:
    ...
    def delItem(self, expr, item):
        self.newCommand(Expr("del_item", [expr, spExpr(item)], get_file_line_no()))
    ...
    def updateSet(self, set, item, add):
        self.newCommand(
            Expr("update_set", [spExpr(set), spExpr(item), add], get_file_line_no())
        )

    def defineLocal(self, local, name, value, mutable):
        self.newCommand(Expr("define_local", [name, value, mutable], get_file_line_no()))
...
class MessageBuilder:
    def __init__(self, added_entry_point):
        if added_entry_point is not None:
            self.name = added_entry_point.name
            self.added_entry_point = added_entry_point
        self.commands = TreeBlock()
...
class Smartml:
    ...
    def runScenario(self, entry_points):
        self.ctx.call("runScenarioInBrowser", entry_points)
...
class Contract:
    ...
    def __setattr__(self, attr, value):
        if attr == "data" and self.entry_points_collecting:
    ...
    def private_lambda(self, name, with_storage, with_operations, f):
        return self.private_variable(name, build_lambda(f, with_storage=with_storage, with_operations=with_operations, contract_self=self))
    ...
    def onoffchain_view(self, fname, name, f, pure, doc, lineNo, kind):
        ov = OnOffchainView(fname, name, f, pure, doc, lineNo, kind)
        self.init_internal()
        ...
        class View:
            ...
            def __call__(self, params=unit):
                return Expr("static_view", [self.ov.name, self.contract_id, spExpr(params)], get_file_line_no()).open_some("View %s is invalid!" % self.ov.name)
    ...
    def init_internal(self):
        ...
        if not hasattr(self, "verbose"):
            self.verbose = False
        if not hasattr(self, "entry_points"):
            self.entry_points = {}
        ...
        if not hasattr(self, "title"):
            self.title = ""
        if not hasattr(self, "entry_points_collected"):
            self.entry_points_collected = False
    ...
    def add_entry_point(self, added_entry_point, set_attribute=True):
        added_entry_point = added_entry_point.message
        if added_entry_point.name is None:
            return
        sp.profile("add_entry_point begin " + added_entry_point.name)
        added_entry_point.contract = self
        mb = MessageBuilder(added_entry_point)
        self.mb = mb
        sp.setMB(mb)
        params = Expr("params", [], line_no=added_entry_point.line_no)
        nargs = pyLen(added_entry_point.args)
        if nargs == 0:
            raise Exception("Entry point '%s' is missing a self parameter (line %s)." % (added_entry_point.name, added_entry_point.line_no.export()))
        elif nargs == 1:
            x = added_entry_point.f(self)
        elif nargs == 2:
            x = added_entry_point.f(self, params)
        else:
            args = [ Expr("attr", [params, a], get_file_line_no()) for a in added_entry_point.args[1:] ]
            x = added_entry_point.f(self, *args)
        if x is not None:
            raise Exception(
                "Entry point failure for %s (line %s): entry points cannot have return statements."
                % (added_entry_point.name, added_entry_point.line_no.export())
            )
        self.mb = None
        sp.setMB(None)
        self.entry_points[added_entry_point.name] = mb
        mb.originate = added_entry_point.originate
        mb.lazify = added_entry_point.lazify
        mb.lazy_no_code = added_entry_point.lazy_no_code
        mb.check_no_incoming_transfer = added_entry_point.check_no_incoming_transfer
        mb.line_no = added_entry_point.line_no
        mb.parameter_type = added_entry_point.parameter_type
        if set_attribute:
            setattr(self, added_entry_point.fname, added_entry_point)
        sp.profile("add_entry_point end " + added_entry_point.name)

    def build_extra_entry_points(self):
        pass

    def collect_entry_points(self):
        if self.entry_points_collected:
            return
        self.contractId = static_contract_id(window.smartpyContext.nextId())
        window.contracts[self.contractId] = self
        self.entry_points_collecting = False
        self.data = sp.getData()
        self.entry_points_collecting = True
        self.entry_points_collected = True
        sp.profile("CollectEntry_Points begin " + self.__class__.__name__)
        for f in dir(self):
            if f == "__setattr__":
                continue
            attr = getattr(self, f)
            if isinstance(attr, Entrypoint):
                ep = attr.message
                attr.message = Added_entry_point(ep.fname, ep.name, ep.f, ep.originate, ep.lazify, ep.lazy_no_code, ep.check_no_incoming_transfer, ep.parameter_type, ep.line_no)
                attr.message.contract = self
        for f in dir(self):
            attr = getattr(self, f)
            if isinstance(attr, PrivateLambda):
                attr._l = self.private_lambda(attr.name, attr.with_storage, attr.with_operations, attr.f)
            if isinstance(attr, OnOffchainView):
                setattr(self, attr.fname, self.onoffchain_view(attr.fname, attr.name, attr.f, attr.pure, attr.doc, attr.line_no, attr.kind))
        for f in dir(self):
            attr = getattr(self, f)
            if isinstance(attr, Entrypoint):
                self.add_entry_point(attr, False)
        self.build_extra_entry_points()
        for f in dir(self):
            attr = getattr(self, f)
            if isinstance(attr, Entrypoint):
                ep = attr.message
                setattr(self, ep.fname, ep)
        # self.smartml = window.buildSmartlmJS(self)
        sp.profile("CollectEntry_Points smartml " + self.__class__.__name__)
        self.entry_points_collecting = False
        ...
        sp.profile("CollectEntry_Points end " + self.__class__.__name__)
    ...
    def export(self):
        ...
        entry_points = {}
        for message in self.entry_points.values():
            if message.added_entry_point.name is not None:
                name = message.added_entry_point.name
                if name in entry_points:
                    raise Exception("Two entry points are called %s (line %s)" % (name, message.added_entry_point.line_no.export()))
                entry_points[name] = message
        ...
        result = "(template_id %s \nstorage %s\nstorage_type (%s)\nentry_points (%s)\nflags (%s)\nprivates (%s)\nviews (%s)\nentry_points_layout %s\ninitial_metadata (%s)\nbalance %s)" % (
            ...
            (
                " ".join(
                    "(%s %s %s %s %s %s %s %s %s)" % (k, str(v.originate), str(v.lazify), str(v.lazy_no_code), str(v.check_no_incoming_transfer), str(pyLen(v.added_entry_point.args) > 1), v.parameter_type, v.line_no.export(), v.export())
                    for (k, v) in sorted(entry_points.items())
                )
            ),
...
class Added_entry_point:
    def __init__(self, fname, name, f, originate, lazify, lazy_no_code, check_no_incoming_transfer, parameter_type, line_no):
        self.fname = fname
        self.name = name
        self.f = f
        ...
        self.check_no_incoming_transfer = check_no_incoming_transfer
        self.parameter_type = parameter_type if parameter_type is not None else "None"
        self.line_no = line_no

    def __call__(self, *args, **kargs):
        if self.contract.entry_points_collecting:
        ...
        self.contract.registered_scenario.add_entry_point(res)
        return res
...
class Entrypoint:
    def __init__(self, message):
        ...
        self.f = message.f
        self.__wrapped__ = self.f

def _add_entry_point(f, fname, name, line_no, originate, lazify, lazy_no_code, check_no_incoming_transfer, parameter_type):
    if name is None:
        name = f.__name__
    if fname is None:
        fname = f.__name__
    ...
    f.__doc__ = "_Entrypoint_. " + doc
    return Entrypoint(Added_entry_point(fname, name, f, originate, lazify, lazy_no_code, check_no_incoming_transfer, parameter_type, line_no))

def entry_point(f = None, name = None, line_no = None, private = False, lazify = None, lazy_no_code = None, check_no_incoming_transfer = None, parameter_type = None):
    originate = not private
    if f is None:
        def g(f):
            return _add_entry_point(f, f.__name__, name, line_no, originate, lazify, lazy_no_code, check_no_incoming_transfer, parameter_type)
        return g
    else:
        return _add_entry_point(f, name, name, line_no, originate, lazify, lazy_no_code, check_no_incoming_transfer, parameter_type)
...
class OnOffchainView:
    def __init__(self, fname, name, f, pure, doc, lineNo, kind):
        self.fname = fname
        self.name = name
        self.f = f
    ...
    def exportDef(self, contract):
        ...
        contract.data = contract_data(contract.contractId)

        return "(%s %s %s %s %s %s %s)" % (self.kind, self.name, nargs > 1, self.line_no.export(), self.pure, json.dumps(self.doc), commands)

def private_lambda(name=None, with_storage=None, with_operations=False, wrap_call=False):
    def transform(f):
        doc = f.__doc__ if f.__doc__ is not None else ""
        doc = "_Private Lambda_. " + doc
        nonlocal name
        if name is None:
            name = f.__name__
        return PrivateLambda(name, with_storage, with_operations, wrap_call=wrap_call, f=f, lineNo=get_file_line_no())
    return transform

def global_lambda(f, name=None):
    print(f"'global_lambda' is deprecated. Please use 'private_lambda' instead. {pretty_line_no()}")
    if name is None:
        name = f.__name__
    def g(self, x):
        return f(x)
    return private_lambda(name=name, with_storage=None, with_operations=False, wrap_call=False)(g)

def offchain_view(pure = False, doc = None, name=None):
    if not isinstance(pure, pyBool):
        raise Exception("The 'pure' parameter must be a boolean. (Did you forget the '()' in 'sp.offchain_view()'?)")
    def apply_offchain_view(f):
        name_ = name if name is not None else f.__name__
        doc_ = doc
        if doc_ is None:
            doc_ = f.__doc__ if f.__doc__ is not None else ""
        f.__doc__ = "_Offchain view_. " + doc_
        return OnOffchainView(f.__name__, name_, f, pure, doc_, get_file_line_no(), "offchain")
    return apply_offchain_view

def onchain_view(pure = False, doc = None, name=None):
    if not isinstance(pure, pyBool):
        raise Exception("The 'pure' parameter must be a boolean. (Did you forget the '()' in 'sp.onchain_view()'?)")
    def apply_onchain_view(f):
        name_ = name if name is not None else f.__name__
        doc_ = doc
        if doc_ is None:
            doc_ = f.__doc__ if f.__doc__ is not None else ""
        f.__doc__ = "_Onchain view_. " + doc_
        return OnOffchainView(f.__name__, name_, f, pure, doc_, get_file_line_no(), "onchain")
    return apply_onchain_view

def sub_entry_point(f, name=None):
    print(f"'sub_entry_point' is deprecated. Please use 'private_lambda' instead. {pretty_line_no()}")
    return private_lambda(name=None, with_storage="read-write", with_operations=True, wrap_call=True)(f)

self = Expr("self", [], LineNo("", -1))
self_address = Expr("self_address", [], LineNo("", -1))
sender = Expr("sender", [], LineNo("", -1))
source = Expr("source", [], LineNo("", -1))
amount = Expr("amount", [], LineNo("", -1))
balance = Expr("balance", [], LineNo("", -1))
now = Expr("now", [], LineNo("", -1))
level = Expr("level", [], LineNo("", -1))
chain_id = Expr("chain_id", [], LineNo("", -1))
none = Expr("variant", ["None", unit], LineNo("", -1))
total_voting_power = Expr("total_voting_power", [], LineNo("", -1))
...
def else_():
    b = CommandBlock(sp)
    sp.newCommand(Expr("else_block", [b]))
    return b

def while_(condition):
    b = CommandBlock(sp)
    sp.newCommand(Expr("while_block", [spExpr(condition), b], get_file_line_no()))
    return b
...
def if_some(condition, name):
    b = CommandBlock(sp)
    sp.newCommand(Expr("if_some_block", [spExpr(condition), name, b], get_file_line_no()))
    value = Expr("open_variant", [spExpr(condition), "Some"], get_file_line_no())
    value.__asBlock = b
    b.value = value
    return value
...
def if_(condition):
    b = CommandBlock(sp)
    sp.newCommand(Expr("if_block", [spExpr(condition), b], get_file_line_no()))
    return b

def for_(name, value):
    value = spExpr(value)
    b = CommandBlock(sp)
    sp.newCommand(Expr("for_group", [name, value, b], get_file_line_no()))
...
def check_signature(pk, sig, msg):
    return Expr("check_signature", [spExpr(pk), spExpr(sig), spExpr(msg)], get_file_line_no())
...
class Lambda:
    def __init__(self, f, with_storage, with_operations, params, tParams, auto_result, contract_self, tstorage):
    ...
    def collectLambda(self, f):
        ...
        commands = TreeBlock()
        sp.mb.currentBlock = commands
        param = Expr("lambda_params", [self.id, self.params, self.tParams], get_file_line_no())
        import inspect
        args = inspect.getargs(f.__code__).args
        nargs = pyLen(args)
        if self.contract_self is None:
            if self.with_storage is None:
                r = f(param)
            else:
                r = f(SelfProxy(), param)
        else:
            if nargs == 1:
                ...
                raise Exception("More than one argument given in private lambda definition.")
        if self.auto_result:
            if r is not None:
                result(r)
        elif r is not None:
        ...
        if self.with_storage not in [None, "read-write", "read-only"]:
            raise Exception("with_storage parameter can only be None, \"read-only\" or \"read-write\".")
        tstorage = Atom("None") if self.tstorage is None else self.tstorage
        r = Expr("lambda", [self.id, self.with_storage, self.with_operations, tstorage, self.params, commands], get_file_line_no())
        sp.mb.currentBlock = currentBlock
        self.mb = prev
        if newMB:
            sp.setMB(None)
...
def build_lambda(f, params="", tParams = None, with_storage=None, with_operations=False, contract_self=None, tstorage=None):
    tParams = sp.types.conv(tParams)
    auto_result = f.__name__ == "<lambda>"
    return Lambda(f, with_storage, with_operations, params, tParams, auto_result, contract_self, tstorage)
...
class Local:
    ...
    def __getattr__(self, attr):
        if attr == "value":
            return Expr("get_local", [self.name], get_file_line_no())
        raise AttributeError(
...
def sapling_test_transaction(source, target, amount, memo_size, bound_data=None):
    ...
    if bound_data is None:
        raise Exception("test_sapling_transaction without bound_data has been deprecated. See the Sapling integration page for a more comprehensive description of the Sapling protocol. http://tezos.gitlab.io/alpha/sapling.html#michelson-verify-update")
    ensure_bytes("test_sapling_transaction bound_data", bound_data)
    return Expr(
        "literal",
        [Expr("sapling_test_transaction", [memo_size, source, target, str(amount), bound_data])],
        get_file_line_no(),
    )
...
def transfer_operation(arg, amount, destination, line_no = None):
    ...
    return Expr(
        "transfer",
        [spExpr(arg, "transfer arguments"), spExpr(amount, "transfer amount"), spExpr(destination, "transfer destination")],
        line_no
    )
...
def contract(t, address, entry_point="", line_no = None):
    ...
    return Expr(
        "contract",
        [entry_point, sp.types.conv(t), spExpr(address)],
        line_no
    )
...
def view(name, address, param, t = None, line_no = None):
    ...
    return Expr(
        "view",
        [name, spExpr(param), spExpr(address), sp.types.conv(t)],
        line_no
    )
...
def set_type(expression, t):
    ...
    return Expr("invalid", ["Invalid expression, sp.set_type should no longer be used as an expression, please use sp.set_type_expr instead. %i" % (get_file_line_no().line_no)], get_file_line_no())

def set_entry_point(ep_name, new_ep):
    entrypoint_map()[entrypoint_id(ep_name)] = new_ep

def has_entry_point(ep_name):
    return entrypoint_map().contains(entrypoint_id(ep_name))
...
class Dynamic_contract:
    ...
    def call(self, entry_point, parameters):
        ...
        self.registered_scenario.add_entry_point(res)
        return res
...
class Scenario:
    ...
    def add_entry_point(self, message):
    ...
    def __call__(self, x):
        ...
        try:
            ...
            if False:
                ...
                if self.app is None:
                    self.app = self.contract.private_variable("get_error", build_lambda(self.get))
...
def ematch(scrutinee, clauses):
    ...
    return Expr("ematch", [scrutinee] + clauses, get_file_line_no())
...
def match_record(x, *fields):
    ...
    sp.newCommand(Expr("match_record", [x] + bse, line_no))
...
def match_tuple(x, *names):
    names = pyList(names)
    line_no = get_file_line_no()
    sp.newCommand(Expr("match_tuple", [spExpr(x)] + names, line_no))
...
class Modify_record(CommandBlock):
    def __init__(self, x, var, sp, line_no):
        ...
        self._x = x
        self._var = var
        self._loc = Expr("get_meta_local", [self._var], self.line_no)
        setattr(self, "__initialized", True)
    ...
    def export(self):
        return (Expr("modify_record", [self._x, self._var, self.commands] + [Expr("binding", b, self.line_no) for b in self.bindings], self.line_no)).export()
...
def modify_tuple(x, *variables):
    ...
    sp.newCommand(Expr("modify_tuple", [x, body] + variables, line_no))
    value = pyTuple([ Expr("get_local", [fld], line_no) for fld in variables])
    body.value = value
    return body
...
def modify(x, variable):
    ...
    line_no = get_file_line_no()
    body = CommandBlock(sp)
    sp.newCommand(Expr("modify", [x, body, variable], line_no))
    value = Expr("get_local", [variable], line_no)
    body.value = value
    return body
...
def contract_entrypoint_map(c):
    check_contract_id("contract_entrypoint_map", c.contractId)
    return Expr("contract_entrypoint_map", [c.contractId], get_file_line_no())

def contract_entrypoint_id(c, ep):
    check_contract_id("contract_entrypoint_id", c.contractId)
    return Expr("contract_entrypoint_id", [c.contractId, ep], get_file_line_no())

def contract_entrypoint(c, ep):
    return contract_entrypoint_map(c)[contract_entrypoint_id(c,ep)]
...
def entrypoint_map():
    return Expr("entrypoint_map", [], get_file_line_no())

def entrypoint_id(ep):
    return Expr("entrypoint_id", [ep], get_file_line_no())
...
class utils:
    ...
    @staticmethod
    def nat_to_mutez(x):
        return mul(set_type_expr(x,TNat), mutez(1))

    @staticmethod
    def nat_to_tez(x):
        return mul(set_type_expr(x,TNat), tez(1))
